...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run TS dataset creation from USDM JSON."
    )
    ...
    parser.add_argument(
        "usdm_file_pos", nargs="?", help="Input USDM JSON file (positional, optional)"
    )
    parser.add_argument(
        "output_file_pos", nargs="?", help="Output TS CSV file (positional, optional)"
    )
    ...
    if not usdm_file or not output_file:
        parser.error(
            "You must provide both --usdm_file and --output_file (or positional equivalents)."
        )
